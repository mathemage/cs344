  ...
  // to sequential reference solution for the exact clamping semantics you should follow.
	...
	for (int r = -halfWidth; r <= halfWidth; ++r) {
		for (int c = -halfWidth; c <= halfWidth; ++c) {
			int pr = min(numRows-1, max(y+r, 0));
			int pc = min(numCols-1, max(x+c, 0));
			float pixel = static_cast<float>(inputChannel[pr * numCols + pc]);
			float filter_value = filter[(r+halfWidth) * filterWidth + c+halfWidth];
			result += pixel * filter_value;
		}
	}
...
{
	const int width = 16;
  const dim3 blockSize(width, width);
